...
Louis de Broglie proposed that matter, like light, also behaves as a wave.
This wave-light behavior of matter leads directly to Werner Heisenberg's uncertainty principle:

$$\sigma_x \sigma_p \ge \hbar / 2$$

-  $\sigma_x$: uncertainty in position
-  $\sigma_p$: uncertainty in momentum
-  $\hbar$: Planck constant $h$ divided by $2\pi$

Essentially, there is a limit to the precision with which we can measure the position and momentum some particle. If we could know a quantity like position ($x$) or momentum ($p$) exactly, its uncertainty would be zero which would violate the uncertainty principle. 
""" 

# ╔═╡ b685a354-71ce-41eb-b70a-3201e50ce32e
# -- max and min values for σ slider
begin
	σmin = 2
	σmax = 50
end;
...
begin
	...
	local minfont = 14
	local maxfont = 60
	local xfont = σ - σmin + 14
	local pfont = σmax - σ + 14
	
	local x0 = 0
		
	# -- define a gaussian
	wf_gauss(x, x0, k, σ) = exp(-((x - x0) / σ) ^ 2) * cos(k * x)
	
	wf(x) = wf_gauss.(x, x0, k, σ)
	
	plot(x, wf(x))

	annotate!(-3L/4, 1.25, text(L"\sigma_x", xfont))
	annotate!( 3L/4, 1.25, text(L"\sigma_p", pfont))
	
end

# ╔═╡ 032444bb-1af6-47e2-b8e6-ed70f7a09ffa
md"""
Given that matter can be well-described by waves,  :
 
$$p = h / \lambda$$

When $\sigma_x$ is small, we have a pretty good idea of where the particle is, but it's not obvious how to define the wavelength which, when the wave packet moves to the left or the right tells us the momentum that the wave has as it moves. Conversely, when $\sigma_x$ is large, we can pretty easily determine the wavelength of the particle (and therefore momentum), but we are less sure about where it is at any point in time. 
"""
...
begin
	...
	# -- define a gaussian
	wf_gaussanimate(x, t, k, width) = exp(-((x - t - x0) / width) ^ 2) * cos(k * x) 
...
manifest_format = "2.0"
project_hash = "c4858f6a9ca2cfcecd746e54be55eb402f52eac2"
